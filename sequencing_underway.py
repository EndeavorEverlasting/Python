...
class DataProcessor:
  ...
  def sequence_check(self, row_id, flr_pln_L_value, flr_pln_N_value,
    ...
    try:
      ...
      # Iterate over each valid floor plan
      for floor_plan, designators in valid_floor_plans.items():
        # Check if the designators within this floor plan are sequential
        if not self.are_designators_sequential(designators):
          ...
          self.highlight_cell(row_id, floor_plan_cell.column, "light_yellow")
  ...
  def are_designators_sequential(self, designators):
    # Define a custom sorting function to handle alphanumeric sorting
    def alphanumeric_sort(designator):
      return [
          int(part) if part.isdigit() else part
          for part in re.split('([0-9]+)', designator)
      ]

    # Sort the designators using the custom sorting function
    sorted_designators = sorted(designators, key=alphanumeric_sort)

    # Check if the sorted designators form a sequential sequence
    for i in range(1, len(sorted_designators)):
      # Check if the current designator is not consecutive with the previous one
      current_designator = alphanumeric_sort(sorted_designators[i])
      previous_designator = alphanumeric_sort(sorted_designators[i - 1])
      if current_designator != previous_designator and \
         (isinstance(current_designator, int) and isinstance(
           previous_designator, int)):
        return False
    return True
  ...
  def flagging_issues(self):
    ...
    for i in range(3, self.ws.max_row + 1):
      ...
      for i in range(3, self.ws.max_row + 1):
        ...
        # Step 2d: Check for sequence issues:
        ...
        self.sequence_check(i, flr_pln_L_value, flr_pln_N_value,
                            department_value, flr_pln_D_value)

      # Step 2e: Check for printer issues:
    ...
    # Step 2f: Combine issue messages into one string
    combined_message = " ".join(msg for msg in issue_messages if msg)

    # Step 2g: Update the "Issues" column
