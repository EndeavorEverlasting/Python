...
#from Components.data_processing import clean_designators, find_duplicates
#from Components.file_io import read_excel_file, save_to_csv
...
def find_duplicates_and_missing_data(input_file, expected_count=None):
  ...
  print("Start of find_duplicates_and_missing_data")
  ...
  for index, row in df.iterrows():
    ...
    if pd.notna(epic_loc) and isinstance(
        epic_loc, str) and ('remote' in epic_loc.lower()
                            or 'remote' in designator):
    ...
    if (pd.notna(designator) and not isinstance(designator, str)
        or not designator[0:].isalpha() or not designator[2:].isdigit()
        or not designator.lower().startswith("remote")):
  ...
  '''
  print("End of find_duplicates_and_missing_data")

  def find_duplicate_designators(row, df):
  ...
  # Add a new column 'Conflict_ID' to store the ID of the record causing a duplicate
  duplicates['Conflict_ID'] = duplicates.apply(
      lambda row: find_duplicate_designators(row, df), axis=1)

  # Assuming the df is the DataFrame with a column 'ID'
  # Replace 'your_id_column' with the actual column name if it's different

  # Identify the first occurrence of each unique ID
  first_occurrences = ~df['ID'].duplicated(keep='first')

  # Output only the rows corresponding to the first occurrences
  first_occurrence_rows = df[first_occurrences]

  # Display the resulting DataFrame
  print(first_occurrence_rows)
  ...
  if not combined_data.empty:
    ...
    def determine_flagging_reason(row):
      ...
      if not designator.lower().startswith("remote"):
    ...
    # Output Tests for Dataframe Shapes:
    print("Shape of df:", df.shape)
    print("Shape of duplicates:", duplicates.shape)
    print("Shape of missing_FlrPlnN:", missing_FlrPlnN.shape)
    print("Shape of entries_to_include:", entries_to_include.shape)
    print("Shape of combined_data:", combined_data.shape)  

    # Output Tests for Intermediate Results:
    print("Shape of df:", df.shape)
    print("Shape of duplicates:", duplicates.shape)
    print("Shape of missing_FlrPlnN:", missing_FlrPlnN.shape)
    print("Shape of entries_to_include:", entries_to_include.shape)
